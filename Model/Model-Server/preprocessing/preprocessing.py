...
# claen
def clean_preprocessing_seq_problem_id2idx(problem_seq):
    return_problem_seq = []
    for problem_id in problem_seq:
        try:
            return_problem_seq.append(int(clean_problem_id2idx[problem_id]))
        except:
            continue
    return return_problem_seq

def clean_preprocessing_seq_idx2problem_id(output):
    return_problem_seq = []
    for idx in output:
        try:
            return_problem_seq.append(clean_idx2problem_id[str(idx)])
        except:
            continue
    return return_problem_seq

def clean_serch_rank(total_problem_seq, lately_problem_seq):
    total_levels = [max(problem_id_to_level[clean_idx2problem_id[str(problem_id_idx)]], 1) for problem_id_idx in total_problem_seq]
    lately_levels = [max(problem_id_to_level[clean_idx2problem_id[str(problem_id_idx)]], 1) for problem_id_idx in lately_problem_seq]
...
def clean_serch_best_tag(problem_seq, top = 3):
    tag_cnt_mat = np.zeros((len(tag_to_idx),))
    for problem_id_idx in problem_seq:
        tags = problem_id_to_tags[clean_idx2problem_id[str(problem_id_idx)]]
        tags = [int(tag_to_idx[tag]) for tag in tags]
        tag_cnt_mat[tags] += 1
